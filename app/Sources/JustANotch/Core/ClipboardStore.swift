...
final class ClipboardStore: ObservableObject {
    ...
    func copyBack(_ id: UUID) {
        ...
        case let .image(fileName):
            guard let dir = imagesDir,
                  let img = NSImage(contentsOf: dir.appendingPathComponent(fileName)) else { return }
            pb.clearContents()
            pb.writeObjects([img])
            selfChangeCount = pb.changeCount
        }
    ...
    private func poll() {
        ...
        // Bỏ qua nội dung nhạy cảm/nhất thời (mật khẩu từ 1Password, Keychain…).
        let ignored: Set<NSPasteboard.PasteboardType> = [
            NSPasteboard.PasteboardType("org.nspasteboard.ConcealedType"),
            NSPasteboard.PasteboardType("org.nspasteboard.TransientType"),
            NSPasteboard.PasteboardType("org.nspasteboard.AutoGeneratedType"),
        ]
        if let types = pb.types, !ignored.isDisjoint(with: types) { return }
