...
/// Audio-like spectrum: slim bars packed tight and centred, each driven by a sum
/// of detuned harmonics shaped for punchy, irregular peaks (not a uniform sine).
/// Ported from the animation prototype. Holds a static frame under Reduce Motion.
struct OrganicWaveform: View {
    var active: Bool
    var reduceMotion: Bool
    var tint: Color = Color(red: 0.96, green: 0.36, blue: 0.33)   // Alcove red
    var bars: Int = 6

    private let amp:   [Double] = [0.55, 0.78, 0.95, 1.0, 0.9, 0.72, 0.58]   // centre-peaked
